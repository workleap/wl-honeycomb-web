...
export function getHoneycombSdkOptions(serviceName: NonNullable<HoneycombSdkOptions["serviceName"]>, apiServiceUrls: PropagateTraceHeaderCorsUrls, options: RegisterHoneycombInstrumentationOptions = {}): HoneycombSdkOptions {
    ...
    if (fetchInstrumentation) {
        autoInstrumentations["@opentelemetry/instrumentation-fetch"] = patchFetchInstrumentationConfig(fetchInstrumentation(instrumentationOptions));
    } else {
    ...
    if (xmlHttpRequestInstrumentation) {
        autoInstrumentations["@opentelemetry/instrumentation-xml-http-request"] = patchXMLHttpInstrumentationConfig(xmlHttpRequestInstrumentation(instrumentationOptions));
    } else {
